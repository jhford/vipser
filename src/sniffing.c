...
format_t format_from_name(const char *name) {
  if (0 == strncmp(JPEG_NAME, name, strlen(JPEG_NAME)) ||
      0 == strncmp(JPEG_NAME2, name, strlen(JPEG_NAME2))) {
    return JPEG;
...
format_t format_from_mime(const char *mime) {
  if (0 == strncmp(JPEG_MIME, mime, strlen(JPEG_MIME))) {
    ...
    return GIF;
  } else if (0 == strncmp(BMP_MIME1, mime, strlen(BMP_MIME1)) ||
             0 == strncmp(BMP_MIME2, mime, strlen(BMP_MIME2))) {
    return BMP;
  }
