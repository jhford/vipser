...
void v_log(level lvl, const char *fmt, ...) {
  ...
  // TODO: This is totally invalid in the case where the two tv_sec members are
  // differnt and likely just generally broken

  fprintf(stderr, "%s - %3lld.%.9ld: ", lvl_string, (long long)diff.tv_sec,
          diff.tv_nsec);
